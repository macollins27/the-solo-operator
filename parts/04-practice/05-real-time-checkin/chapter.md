...
Architecture in one paragraph: a member taps "check me in" → the API records an `EventCheckin` row → the API publishes a `checkin.created` event to a channel scoped to that event → any browser subscribed to that channel receives the event and updates its UI. The pub/sub layer for MembershipKit's default stack is Soketi (a self-hosted Pusher-compatible WebSocket server) — concrete-but-swappable; alternatives include Centrifugo, native Pusher Channels, or Ably if Soketi's maintenance activity drops off. Soketi runs locally in Docker; no SaaS needed.
