...
**Drill 2 — Scaffold the Next.js app.** Inside your fork, have Claude scaffold a Next.js app at `student/canonical-project/` (TypeScript, App Router, no Tailwind/ESLint/examples). Caveat: `pnpm create next-app` (and the equivalent npm/yarn scaffolders) expect the target directory to NOT already exist OR to be completely empty — if you pre-created the folder and it contains anything, the scaffolder will refuse. Easiest path is to let the scaffolder create the folder itself, or `rm -rf student/canonical-project/` first if you created it accidentally. After scaffolding, the folder should contain `package.json`, `app/`, and a few config files. Don't touch the reference `canonical-project/` at the repo root.
